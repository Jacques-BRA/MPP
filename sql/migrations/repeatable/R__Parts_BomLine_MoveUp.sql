-- =============================================
-- Procedure:   Parts.BomLine_MoveUp
-- Author:      Blue Ridge Automation
-- Created:     2026-04-14
-- Version:     1.0
--
-- Description:
--   Swaps a BomLine's SortOrder with the nearest-lower sibling within
--   the same BomId. No-op (status=1, message "already first") if already
--   first. Rejects if the parent BOM is Published or Deprecated.
--
-- Parameters (input):
...
--
-- Parameters (output):
--   @Status BIT            - 1 on success (or already-first no-op), 0 on failure.
...
-- =============================================
CREATE OR ALTER PROCEDURE Parts.BomLine_MoveUp
    @Id        BIGINT,
    @AppUserId BIGINT,
    @Status    BIT            OUTPUT,
...
BEGIN
    ...
    DECLARE @ProcName NVARCHAR(200) = N'Parts.BomLine_MoveUp';
    ...
    BEGIN TRY
        ...
        -- Find the nearest sibling above
        ...
        EXEC Audit.Audit_LogConfigChange
            ...
            @Description       = N'BomLine moved up.',
            @OldValue          = NULL,
            @NewValue          = @Params;
        ...
        SET @Status  = 1;
        SET @Message = N'BomLine moved up successfully.';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
