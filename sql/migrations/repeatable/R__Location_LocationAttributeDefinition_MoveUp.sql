-- =============================================
-- Procedure:   Location.LocationAttributeDefinition_MoveUp
-- Author:      Blue Ridge Automation
-- Created:     2026-04-13
-- Version:     2.0
--
-- Description:
--   Moves a LocationAttributeDefinition up in the sort order by swapping
--   SortOrder with the nearest active sibling above (lower SortOrder).
--   No-op (Status=1) if already at the top position.
--
-- Parameters (input):
...
BEGIN
    ...
    BEGIN TRY
        ...
        EXEC Audit.Audit_LogConfigChange
            ...
            @Description       = N'LocationAttributeDefinition moved up.',
            @OldValue          = @OldValue,
            @NewValue          = @NewValue;
        ...
        SET @Status  = 1;
        SET @Message = N'LocationAttributeDefinition moved up successfully.';
        SELECT @Status AS Status, @Message AS Message;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
