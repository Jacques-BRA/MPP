-- =============================================
-- Procedure:   Quality.QualitySpecAttribute_MoveUp
-- Author:      Blue Ridge Automation
-- Created:     2026-04-14
-- Version:     2.0
--
-- Description:
--   Swaps SortOrder with the previous sibling (lower SortOrder).
--   Rejects if parent version is published/deprecated or if
--   the attribute is already at the top.
--
-- Parameters (input):
--   @Id BIGINT        - Required.
...
-- =============================================
CREATE OR ALTER PROCEDURE Quality.QualitySpecAttribute_MoveUp
    @Id        BIGINT,
    @AppUserId BIGINT
AS
BEGIN
    ...
    DECLARE @ProcName NVARCHAR(200) = N'Quality.QualitySpecAttribute_MoveUp';
    ...
    BEGIN TRY
        ...
        BEGIN
            SET @Message = N'Attribute is already at the top.';
            EXEC Audit.Audit_LogFailure
                @AppUserId = @AppUserId, @LogEntityTypeCode = N'QualitySpecAttribute',
                @EntityId = @Id, @LogEventTypeCode = N'Updated',
                @FailureReason = @Message, @ProcedureName = @ProcName,
        ...
        EXEC Audit.Audit_LogConfigChange
            ...
            @Description       = N'Quality spec attribute moved up.',
            @OldValue          = NULL,
            @NewValue          = @Params;
        ...
        SET @Status  = 1;
        SET @Message = N'Attribute moved up.';
    SELECT @Status AS Status, @Message AS Message;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
