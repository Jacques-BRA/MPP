...
### 8:30am — Trim Shop: LOT Moves Through the Plant — and Changes Identity
...
**Part-identity change (OI-11):** MPP parts change part number as they cross into the Trim Shop — the cast part is `5G0-CAST-4102`, the trim part is `5G0-TRIM-4102`. The MES prompts: *"This LOT is 5G0-CAST-4102. Receive as 5G0-TRIM-4102?"*. The operator confirms. Under the hood this is a normal **1-line BOM consumption**: the trim part's BOM has a single line pointing back to the cast part at QtyPer=1. The MES creates a new destination LOT under the trim part, writes a `Workorder.ConsumptionEvent` (consumed cast / produced trim, piece-for-piece), and a `Lots.LotGenealogy` row with `RelationshipType = Consumption` links the two LOTs. Honda backward trace walks the genealogy from shipped trim LOT → cast LOT → die / cavity / Carlos — same code path as any assembly consumption (FDS-05-033). *(An earlier v0.10 FDS draft added a dedicated `Parts.ItemTransform` table; it was removed as redundant with `ConsumptionEvent`.)*
