...
**Part-identity change (OI-11):** MPP parts change part number as they cross into the Trim Shop — the cast part is `5G0-CAST-4102`, the trim part is `5G0-TRIM-4102`. The MES prompts: *"This LOT is 5G0-CAST-4102. Receive as 5G0-TRIM-4102?"*. The operator confirms. The MES creates a new destination LOT under the trim part number, the source cast LOT decrements, and a `Parts.ItemTransform` row captures the bridge so Honda queries resolve across the rename (FDS-05-033 through FDS-05-035). Genealogy still flows through `LotGenealogy`; `ItemTransform` layers the part-number change on top so a backward trace from a shipped trim LOT cleanly threads back to the cast LOT.
